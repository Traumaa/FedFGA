...
class Trainer():
    def __init__(self, args, loader, agent_list, ckp):
        ...
        self.merger_epoch = 1 # 每多少轮进行点火率选择通道
    ...
    def train(self):
        ...
        while True:
            # #极端情况
            # # agent_budget = np.full(10, 0.25)
            # agent_budget = np.full(10, 1)
            # break
    ...
    def test(self):
        ...
        self.top_channels_indices = self.tester.top_channels_indices
        if epoch <= self.total_epochs - 30 and epoch % self.merger_epoch == 0:
            print("开始根据点火率聚合拆分通道")
            self.sync_firerate() # 按照点火率索引给所有客户端和测试客户端分配当前参数，接着进行下一轮训练，客户端使用当前分配到的通道索引进行训练，然后聚合
            print("聚合拆分完成")
    ...
    def sync_firerate(self):
        ...
        conv = self.conv_id
        epoch = self.epoch
        print(f"每{self.merger_epoch}轮按照点火率选择通道聚合一次，当前轮次：", epoch)
        # 合并网络参数代码
        weights_dict = {}
        ...
        mergde_tensor = self.merge_tensors(weights_dict)
        # print("合并后的张量形状为", mergde_tensor.shape())
        # 将合并后的张量赋值给1.0网络
        anchors[1.0][conv] = mergde_tensor

        # 筛选出通道索引，接下来根据通道索引对1.0网络的conv2.conv.1.weight参数进行拆分操作
        ...
        '''
        # 初始化一个字典，用于存储网络分数为1.0的网络的conv2.conv.1.weight参数
        anchor_1_0 = {}
        anchor_1_0[conv] = anchors[1.0][conv]

        # # 遍历测试客户端的模型参数字典，找到网络分数为1.0的模型
        ...
        print(f"取出来的{conv} tensor in anchor_1_0 shape:", anchor_1_0[conv].shape)
        ...
        for net_f in self.args.fraction_list:  # 对不同分数的网络进行处理
            ...
            if net_f == 0.25:  # 对0.25网络执行最后一层参数拆分复制操作
                anchors[net_f][conv] = self.split_tensor_by_indices_25(anchor_1_0, top_channels_indices_25)
                print("Shape of 0.25 tensor:", anchors[net_f][conv].shape)
            if net_f == 0.5:
                # 对于'conv2.conv.1.weight'键，执行赋值0.50操作
                anchors[net_f][conv] = self.split_tensor_by_indices_50(anchor_1_0, top_channels_indices_50)
                print("Shape of 0.50 tensor:", anchors[net_f][conv].shape)
            if net_f == 0.75:
                # 对于'conv2.conv.1.weight'键，执行赋值0.75操作
                anchors[net_f][conv] = self.split_tensor_by_indices_75(anchor_1_0, top_channels_indices_75)
                print("Shape of 0.75 tensor:", anchors[net_f][conv].shape)
            if net_f == 1:
                # 对于'conv2.conv.1.weight'键，执行赋值1.0操作
                anchors[net_f][conv] = self.split_tensor_by_indices_10(anchor_1_0, self.top_channels_indices)
                print("Shape of 1.00 tensor:", anchors[net_f][conv].shape)
    ...
    def split_tensor_by_indices_25(self, anchor_1_0, top_channels_indices_25):
        # 获取 anchor_1_0['conv2.conv.1.weight'] 的形状
        conv = self.conv_id
        original_shape = anchor_1_0[conv].shape
        tensor = anchor_1_0[conv]
        ...
        for i, idx in enumerate(top_channels_indices_25):
            new_tensor[i, :, :, :] = tensor[idx, :64, :, :]

        return new_tensor

    def split_tensor_by_indices_50(self, anchor_1_0, top_channels_indices_50):
        # 获取 anchor_1_0['conv2.conv.1.weight'] 的形状
        conv = self.conv_id
        original_shape = anchor_1_0[conv].shape
        tensor = anchor_1_0[conv]
        ...
        # 根据 top_channels_indices 进行拆分
        for i, idx in enumerate(top_channels_indices_50):
            new_tensor[i, :, :, :] = tensor[idx, :128, :, :]

        return new_tensor

    def split_tensor_by_indices_75(self, anchor_1_0, top_channels_indices_75):
        # 获取 anchor_1_0['conv2.conv.1.weight'] 的形状
        conv = self.conv_id
        original_shape = anchor_1_0[conv].shape
        tensor = anchor_1_0[conv]
        ...
        # 根据 top_channels_indices 进行拆分
        for i, idx in enumerate(top_channels_indices_75):

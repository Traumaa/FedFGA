...
class Loss(nn.modules.loss._Loss):
    ...
    def forward(self, prediction, label_onehot, label, train=True):
        losses = []
        for i, l in enumerate(self.loss):
            if l['function'] is not None:
                loss = l['function'](prediction, label_onehot)
    ...
    def accuracy(self, prediction, label):
        # prediction.shape = [B, numofneu, numofclients] ori:[32, 10] => [32, 10, 10]
        # ori: label = [B] => [32,1] spiking: [B, 10] => [B, 10, 1]
        topk = (1, 5)
        ...
        # train_samples = 0
        # train_acc = 0
        # train_acc += (prediction.argmax(1) == label).float().sum().item()
        # train_samples += label.shape(0)
        # res = train_acc/train_samples

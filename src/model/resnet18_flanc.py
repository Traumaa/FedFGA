    ...
    def forward(self, x):
        if self.group == 1:
            ...
            for xi in torch.split(x, self.basis_size, dim=1):
                x1 = F.conv2d(input=xi, weight=self.weight, bias=self.bias, stride=self.stride, padding=self.kernel_size // 2)
                print(1,x1.shape)
                x_list.append(x1)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        print('1 conv1 ResNet forward 输出：', x.shape)
        #x = self.maxpool(x)
        #print(x.shape)
        x = self.layer1(x)
        print('2 layer1 ResNet forward 输出：', x.shape)

        x = self.layer2(x)
        print('3 layer2 ResNet forward 输出：', x.shape)

        x = self.layer3(x)
        print('4 layer3 ResNet forward 输出：', x.shape)

        x = self.layer4(x)
        print('5 layer4 ResNet forward 输出：', x.shape)
